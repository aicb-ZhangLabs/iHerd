...
class GCN:
    ...
    def build_tf_graph(self):
        ...
        # placeholders
        input_embed = tf.compat.v1.placeholder(tf.float32, shape=[None, self.embed_dim], name="input_embed")  # None: node_num
        gcn_A = tf.compat.v1.sparse_placeholder(tf.float32)  # node_num * node_num
        expected_embed = tf.compat.v1.placeholder(tf.float32, shape=[None, self.embed_dim], name="expected_embed")

        curr = input_embed

        for i in range(hidden_layer_num):
            W_hidd = glorot((self.embed_dim, self.embed_dim), name="W_" + str(i)) # random initialization of hidden weights
            curr = act_func(dot(dot(gcn_A, curr, sparse=True), W_hidd))
            vars_arr.append(W_hidd)

        pred_embed = tf.math.l2_normalize(curr, axis=1)  # this normalization is necessary.
        loss = 0.0
        loss += tf.compat.v1.losses.mean_squared_error(expected_embed, pred_embed) * self.embed_dim
    ...
    def refine_embedding(self, coarse_graph=None, fine_graph=None, coarse_embed=None):
        ...
        embedding= initial_embed
        if fine_graph.node_num < self.embed_dim:
           m=int(self.embed_dim / fine_graph.node_num)
...
class GraphSage:
    ...
    def build_tf_graph(self):
        ...
        # placeholders
        input_embed = tf.compat.v1.placeholder(tf.float32, shape=[None, self.embed_dim], name="input_embed")  # None: node_num
        neigh_vecs = tf.compat.v1.placeholder(tf.float32, shape=[None, self.embed_dim],
        ...
        max_pool = tf.math.reduce_max(neigh_reshape, axis=1)

        neigh_wgts = glorot((mlp_dim, self.embed_dim), name="neigh_wgts")
        self_wgts = glorot((self.embed_dim, self.embed_dim), name="self_wgts")
        from_neighs = tf.linalg.matmul(max_pool, neigh_wgts)
        from_self = tf.linalg.matmul(self_vecs, self_wgts)
        if not concat:
        ...
        pred_embed = tf.math.l2_normalize(output, axis=1)
        loss = 0.0
        loss += tf.math.reduce_mean(tf.math.reduce_sum(tf.math.square(expected_embed - pred_embed), axis=1))
